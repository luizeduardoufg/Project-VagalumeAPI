...
const requestMusic = (artist, song) => {
    return $.getJSON(
        ...
        console.log(`The music ${song} is not brazilian.`)
        return null
    })
}
        ...
        for (let art of arts) {
            ...
            try{
                for(let song of songs){
                    if (fs.existsSync(path.join(__dirname, 'Genders', gen, art, `${song.desc.replace(slash, '')}.json`))){
                        console.log(`The music ${song.desc} already exists.`)
                        continue
                    }
